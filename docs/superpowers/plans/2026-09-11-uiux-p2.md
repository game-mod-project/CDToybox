...
Expected: `421 tests, 0 failures` (정렬 5 + 캐시 2 + 색인 3 + 정렬 사양 2 = 13).
...
- [ ] **Step 5: 빌드·테스트** — Expected `423 tests, 0 failures`.
...
- [ ] **Step 4: 빌드·테스트** — `423 / 0`. 소켓 칸의 기존 `PushID(k)` 는 라벨 `##s%d` 로 대체됐으므로 없어야 한다.
...
- [ ] **Step 8: 문서 (`docs/STATUS.md`)** — `src/render/` 줄의 공통 위젯 목록에 `table_sort 표 정렬 · view_cache 뷰 캐시` 를 더한다. `tests/` 줄의 개수를 실측값으로(`425개 (전부 통과 - … 실측 2026-09-11)`).

- [ ] **Step 9: 빌드·테스트** — Expected `425 tests, 0 failures`.
...
- **테스트 수**: 408 → 421(T1) → 423(T2) → 424(T8) → 425(T9).
