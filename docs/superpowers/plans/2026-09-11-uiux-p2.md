...
- 표 정렬: 새로 정렬을 다는 표는 `ImGuiTableFlags_Sortable | ImGuiTableFlags_SortTristate`. 정렬은 뷰(포인터·색인 벡터)에만 걸고 원본 벡터는 건드리지 않는다. 해제(`SpecsCount == 0`, `SortSpec::column < 0`)면 원래 순서. 비교는 `render/table_sort.h` 의 `cmp3` 로만. **tristate 표는 처음 뜬 프레임에 `SpecsCount == 0`(정렬 없음 = 원래 순서)** 이다 — 기본 정렬을 원하는 열에만 `ImGuiTableColumnFlags_DefaultSort` 를 단다(리뷰 확인, imgui_tables.cpp 3189행).
