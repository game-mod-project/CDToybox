...
> **정리 (2026-09-21) — ✅ 해결.** 파트 16종이 연구 없이 지급·장착된다(`STATUS.md` §1.21,
> `2026-09-16-story-vehicle-wheel.md`) — "연구가 장착을 막는가" 는 막지 않았다. 재소환
> 쿨다운은 여기 50분(사용자 실측), `2026-09-15-dragon-call-position.md` 는 60분
> (`_callMercenaryCoolTime` 3600초)이라 적었다 — 두 값이 다르다(확인 전).
