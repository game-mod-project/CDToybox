...
> **정리 (2026-09-21) — ✅ 마을 · 호출 위치는 해결.** 게임 확인 2026-09-17 · 09-18
> (`STATUS.md` §1.21.1 · §1.21.2). 머리말의 "아직 게임에서 풀어 본 것은 없다" 는 첫
> 작성 때 이야기다. **남은 것 둘** — 지도의 붉은 구역(`IsVehicleAllowedInEnteredRegion`,
> `STATUS.md` §1.21.1)과 §8 보스룸(⏳ 미해결, `2026-09-21-boss-room-action-limit.md` §10).
...
## 8. 보스룸 — **미해결**, 그러나 범위는 크게 좁혔다 (2026-09-19)

> 이 절은 처음에 "## 6." 으로 붙어 번호가 겹치고 "## 7" 뒤에 있었다.
> §8 로 바로잡았다(2026-09-21). 전 과정 상세는
> `specs/2026-09-19-mount-vitals-authority.md` §7, 그 뒤 진행은
> `src/game/callcheck.h`(동료 세션 #80~#86).
>
> ➡️ **2026-09-21** — "캐릭터 행동 제한 목록의 허용 스킬그룹 목록이 호출 스킬
> 판정(`0x362DF0`, 행동 차트 조건 0xED)을 막는다" 는 가설로 토글을 넣었으나 **게임에서
> 반증**됐고 토글은 걷어냈다(#91~#93). 아래 §8-2 의 두 패치가 무반응이었던 것은 조건
> **체계**가 달라서다(AI 차트 ≠ 행동 차트) — 이것은 코드 사실로 그대로다.
> `specs/2026-09-21-boss-room-action-limit.md` §10.

사용자 신고: "A.T.A.G. 가 보스룸에서 호출도 안 되고 강제 하차된다".

### 8-1. 실측으로 못박은 것

| 관찰 | 어떻게 쟀나 | 무엇이 배제되나 |
|---|---|---|
| **문구가 없다** | 화면 | `failmessageinfo` 조건층 전부. 거기 걸리면 조건마다 문구가 뜬다(§1) |
| **휠은 열리고 선택도 된다** | 화면 | UI 가 조건을 보고 끄는 층 |
| **드래곤·말·동물까지 전부 안 된다** | 화면 | 탈것 **종류별** 조건 전부(`IsVehicleAllowedInEnteredRegion` 계열) |
| **메시지가 하나도 안 나간다** | 보스룸에서 3번 눌러 로그 확인 — 캡처 0건 | 서버 거부. 클라이언트가 보내지도 않는다 |
| **액터 메모리가 풀린다** | 값을 쓴 액터 주소를 나중에 읽으니 `+0x60` 이 float 1.0 | "내리기" 가 아니라 걷어내기(또는 재생성) |
| **스테이지 전환이 플레이어 액터를 재생성한다** | 보스룸을 나온 뒤 `[actor+0x68]` 이 쓰레기 | 주소 기반 안팎 비교는 못 쓴다 |
| **휠·예약슬롯 상태는 안팎이 동일** | `probe wheel` 을 안/밖에서 각각 — 28항목 필드가 한 칸도 안 다름 | 예약 슬롯 층 |

### 8-2. 시험하고 **반증된** 후보 둘

둘 다 패치가 실제로 걸린 것을 로그로 확인한 뒤 눌러 봤고, 아무 변화가 없었다.
**자리가 틀린 것이 아니라 조건이 틀린 것이다.** *(2026-09-21 정정: 조건이 아니라
**체계**가 달랐다 — 둘 다 AI 차트 조건이라 캐릭터 행동 차트가 쓰는 조건 표에 없다.
`specs/2026-09-21-boss-room-action-limit.md` §5.)*

| 후보 | 패치 | 결과 |
|---|---|---|
| `AICondition_BlockByExclusiveStage` (판정 RVA 0x22EA800) | `xor bl,1` → `mov bl,1;nop` @0x22EA84A | 로그 `켬: 0x1422EA84A` · **무반응** |
| `AICondition_CheckIsRideLimit` (판정 RVA 0x22E6B10) | `jne` → `jmp` @0x22E6B4B (2바이트, 프로브로 즉시 시험) | **무반응** |

> 첫째는 한때 토글로 넣었다가 **걷어냈다**(2026-09-19). 아무 일도 안 하면서
> 코드만 건드리는 토글은 남기지 않는다.

### 8-3. 막힌 길

- **메시지 클래스 → 보내는 쪽**: `TrocTrUseItemReserveSlotReq`·
  `TrocTrFrameEventCallMercenaryReq` 의 vtable 참조는 각각 한 곳뿐이고 그것은
  67바이트짜리 **팩토리 썽크**다. 그 썽크를 `call` 로 부르는 자리가 **0곳** —
  메시지 표에 등록해 간접 호출한다. 클래스에서 거슬러 오르는 길은 없다.
- **행동 제한 표**: `actionrestrictionorderinfo` 는 2.5KB에 탈것 관련 문자열이
  없다. `stageinfo`(28MB)의 히트는 NPC 이름뿐이다.
- **CommonActorManager**: 인스턴스 2개에 액터가 하나도 없다(서버 액터를 싸게
  잡으려던 길이었다).

### 8-4. 다음에 할 일

남은 자리는 **"휠에서 골랐다" 와 "요청을 만든다" 사이**다. 거기서 조용히
빠진다. 프로브만으로는 한 번 재는 데 몇 분이고 스테이지를 드나들 때마다 주소가
죽어 못 쫓는다.

**모드 안에 진단을 넣는 것이 맞다.** 모드는 같은 프로세스라 플레이어를 매
프레임 싸게 잡는다. 버튼 하나로 그 구간의 상태를 로그에 뱉게 하면 안팎 비교가
몇 초로 끝난다. 빌드·배포·재시작 한 번이 드는 대신 그 뒤로는 빠르다.
