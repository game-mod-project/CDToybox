...
> **정리 (2026-09-21) — ✅ 해결 (값은 옛 빌드).** 인벤토리 직행 지급
> (`CreateItemFromTrItemValueCheatReq`)은 동작이 확인됐고 지금 지급의 바탕이다
> (`STATUS.md` §6). 단 여기의 **메시지 ID 와 RVA 는 옛 빌드 값**이다 — 2944 에서
> 치트 33종이 전부 재번호됐다(`2026-09-18-game-update-2944.md`,
> `TROUBLESHOOTING.md` §5.1.1). "문 하나를 열면 35개가 전부 열린다" 는 같은 문서
> 뒤쪽이 뒤집었고 쓸 수 있는 것은 10개뿐이다(`STATUS.md` §6). 바닥 스폰 경로는
> 2026-09-19 걷어냈다(`2026-09-19-mount-vitals-authority.md` §6).
