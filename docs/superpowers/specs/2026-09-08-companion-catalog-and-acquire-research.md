# 탈것·특수탈것·반려동물 — 목록과 오버레이 획득 경로 리서치 (2026-09-08)

> **정리 (2026-09-21) — ✅ 해결.** 원하는 종을 얻는 문제는 종 교체로 풀렸다(`STATUS.md`
> §1.10). 하나는 확인 전이다 — §6.18 의 "`+0x148` = 소유자" 와 `STATUS.md` §1.21 의
> "`+0x148` = 올려 둔 휠 칸" 이 어긋난다. 어느 쪽도 상대를 틀렸다고 적지 않았다
> (`TROUBLESHOOTING.md` §8).

주제: **오버레이에서 원하는 탈것·특수탈것·반려동물을 골라 얻는 것.**
기존 문서 전부를 다시 읽고, 게임을 켜지 않고 되는 정적 분석으로 새 길을
찾았다.

> **먼저 읽을 것:** 이 문서는 시간순 조사 기록이라 §6.5~§6.19 가
> 덧붙으며 길어졌고, 중간에 **틀렸다가 나중에 정정된 단정**이 그대로
> 남아 있다. 결론과 최종 사실만 필요하면
> `2026-09-10-companion-species-swap-session.md` 를 본다 — 정리본이다.
> 여기는 "어떻게 그 결론에 이르렀나"를 남기려고 지우지 않은 기록이다.

이 문서는 조사 기록이다. **구현·배포는 하지 않았다.** 게임이 꺼져 있어
라이브 검증은 한 건도 못 했으므로, 아래 "새로 연 것"은 전부 **정적 근거**
이고 "신규 후보"는 **가설**이다. 화면에서 보기 전까지는 가설이라는 것이
이 프로젝트의 반복된 교훈이다(`2026-09-08-companion-register-session.md` §6).

---

## 1. 구성 확인 (실측 2026-09-08)

| 항목 | 상태 |
|---|---|
| 저장소 | `E:\CDToybox`, 브랜치 `develop`, HEAD `91547d8` |
| 작업 트리 | 깨끗함 (추적 안 되는 참고 zip·CT 표만 있음) |
| 빌드 산출물 | `build\xinput1_4.dll` (2026-09-08 20:45) |
| 배포본 | 게임 `bin64\xinput1_4.dll` — **md5 동일** (`96c3ee39…`) 재배포 불필요 |
| 게임 | **실행 중이 아님.** probe 실측 불가 |
| 게임 exe | `CrimsonDesert.exe` 2026-09-04 13:56, 379MB |
| 정적 분석 | `tools/rtti/{find_class,fields,disasm}.py` 동작 확인 (Python 3.14.3) |

### 곁가지 결함 — 로그가 낙사 경고로 덮인다

`bin64\CDToybox.log` 728줄 중 **289줄(40%)** 이 같은 줄이다.

```
WARN  낙사: 사이트 시그니처가 유일하지 않다(0) - 설치 안 함
```

2.4초마다 재시도하며 매번 WARN 을 쓴다. 낙사 훅은 이 빌드에서 참고 AOB 가
0히트라 어차피 설치되지 않는다. **한 번 실패하면 그만두고 한 줄만 남겨야
한다.** 지금은 실제 조사 로그가 이 줄에 묻힌다. 고칠 자리는 `nofall` 설치를
재시도하는 루프다.

---

## 2. 목록 — 무엇을 얻을 수 있나

원본: `2026-09-05-catchable-companions.md` (라이브 메모리에서 추출,
캐릭터 표 7250행 × 용병 표 21행 교차). 사용자가 물은 세 갈래로 다시 접었다.

| 게임 안 이름 | 엔진 타입 | 캐릭터 수 | 그중 야생(포획 대상) |
|---|---|---|---|
| 탈것 — 말 | `Vehicle_Horse` (행 1) | 219 | **88** |
| 특수 탑승물 | `Vehicle_Special` (행 5) | 63 | **47** |
| 탈것 — 짐승 | `Vehicle` (행 6) | 69 | **35** |
| 반려동물 | `Pet` (행 9) | 230 | **229** |
| 가축 | `Domestic` (행 11) | 33 | 33 |
| 마차 | `Wagon` (행 8) | 75 | 0 |
| 물고기 | `Fish` (행 12) | 48 | 41 |
| 곤충 | `Insect` (행 13) | 76 | 0 |
| 드래곤 | `Vehicle_Dragon` | 1 | 0 |
| ATAG | `Vehicle_WarMachine(_Raptor)` | 3 | 0 |

- 판별식은 `CharacterInfo._mercenaryInfo`(+0xBE) ≠ 0xFFFF **그리고** 내부
  이름에 `_Wild`. `_isCatchable`(+0x148) 은 거의 모든 행이 1 이라 쓸모없다.
- `_Domestic`(마구간판)·`_Saddle`·`_Bagpack`·`Riding_*` 는 종이 아니라
  **상태 변형**이다. 목록에서 접어야 한다.
- **드래곤·ATAG·마차·곤충은 포획 대상이 아예 없다.** 스토리 부여·제작·구매다.
  "아무 종이나"를 목표로 잡을 때 여기가 천장이다.
- 전설마 3종(White 31377 / Black 31378 / RedHare 31379)은 목록에 있으나
  **2338 이 거부한다**(코드 `0x97AE29C9`, 실측 2026-09-06).

오버레이는 이 목록을 이미 보여 준다 — `roster_panel.cpp` 의 **동반자** 탭
(용병 타입 콤보로 접기) 과 **캐릭터** 탭. 표시명은 현지화 필드 `0x30`.

---

## 3. 획득 경로 — 지금 되는 것과 안 되는 것

문서 여덟 편(`2026-09-04-vehicle-pet-review` … `2026-09-08-companion-register-session`)
의 결론을 하나로 접는다.

| 경로 | 종 선택 | 상태 |
|---|---|---|
| **2338** `HireMercenaryToTarget` | **자유** | ✅ **되는 유일한 길.** 단 그 개체가 월드에 있어야 한다 |
| 2454 `HireMercenaryFromInventory` | 아이템이 결정 | ⚠️ 부적 6종이 한계. 인자가 (컨테이너, 슬롯) 이라 종을 못 고른다 |
| 2386 `CatchBySummon` | 부화체만 | ⚠️ 이름과 달리 야생 포획이 아니다. 알→둥지→5분 절차 전용 |
| 2988 `SpawnCharacterCheat` | 자유 | ⛔ **금지.** 몇 번은 돌지만 반복하면 게임이 멈춘다(2026-09-08 실측) |
| 용병 치트 3종 | — | ⛔ 이 빌드에서 몸통이 비어 있다 |

그래서 **남은 문제는 하나로 좁혀져 있다: 원하는 종을 월드에 띄우는 것.**
띄우기만 하면 2338 이 등록한다. 그리고 띄우는 유일한 알려진 수단(2988)이
금지선 너머에 있다. 이것이 이 세션 이전의 벽이다.

### 오버레이가 지금 제공하는 것

`roster_panel.cpp` 탭 6개:

- **동반자** — 동반자 타입 캐릭터 목록(타입별 필터)
- **근처** — 살아 있는 액터 목록 + 줄마다 **획득** 버튼(2338 구동) · **거두기**(2386)
- **탈것** / **용병 타입** / **캐릭터** — 정적 표 뷰어
- **동반자 아이템** — 부적 6종·알 4종 등 13개 지급 버튼(줄마다 실측/미확인 표시)

**없는 것:** 내가 가진 동반자 목록(소유 명부) 뷰. `2026-09-05-vehicle-pet-add-review`
§2.5 가 D안으로 제안했으나 "레코드 구조 미해독"으로 멈춰 있다.

---

## 4. 오늘 새로 연 것 (정적 근거)

### 4.1 2338 은 액터가 가리키는 "캐릭터 행 번호"를 그대로 쓴다 — 전 구간 확인

고용 작업 `0x2ADE280` 을 처음부터 끝까지 읽었다. 종을 정하는 값이
**액터 사슬에서만** 나온다.

```
0x2ADE3AB   rax = [대상액터 + 0x68]
0x2ADE3AF   rcx = [rax + 0x20]
0x2ADE3B7   cx  = word [rcx + 0x30]      ← 캐릭터 표 행 번호
0x2ADE3C9   call 0x383200                 행 → CharacterInfo 레코드
0x2ADE3CE   bx  = word [레코드 + 0x6A]     (탈것 전용 부속 키로 보임)
0x2ADE3E3   rcx = 레코드 + 0xBE            _mercenaryInfo
0x2ADE3EF   == 0xFFFF 면 동반자 아님 → 건너뜀
0x2ADE3F4   call 0x382E40                 용병 표 조회
0x2ADE3F9   [+0x20] == 2 (Vehicle) 이면 부속 등록 루프
   …
0x2ADE46F   ax  = word [[액터+0x68]+0x20 + 0x30]   ← 같은 행 번호를 다시
0x2ADE473   0x58바이트 레코드의 +0x00 에 넣고 배열에 append
   …
0x2ADE634   bx  = word [[액터+0x68]+0x20 + 0x30]   ← 또 같은 자리
0x2ADE665   r8d = bx
0x2ADE673   call 0x2097BC0                 ← 자격 검사(행 번호를 받는 그 함수)
0x2ADE785   call 0x26B4FD0                 ← 실제 등록 1단계(레코드 배열 전달)
0x2ADE825   call 0x26B5F80                 ← 2단계
```

즉 **행 번호가 세 곳에서 읽히고 셋 다 같은 사슬**이다. 액터의 겉모습·
스탯·이름이 아니라 이 u16 하나가 "무엇으로 등록되는가"를 정한다.
이 사슬은 `actors.{h,cpp}` 가 이미 읽고 있는 그 자리다.

### 4.2 거부 코드 두 개의 자리

문서가 "이름 미상"으로 남겨 둔 두 코드의 출처를 찾았다. 둘 다 런타임
초기화 전역이다.

| 전역 RVA | 언제 |
|---|---|
| `0x6BB7AE8` | 핸들로 액터를 못 찾았을 때 (`0x2ADE33C`) |
| `0x6BB8A18` | 액터 종류 바이트 `[[액터+0x88]+1]` 이 **4·5·6 이 아닐 때**, 또는 `[[액터+0x68]+0x118]+0x18` 이 0 이 아닐 때 (`0x2ADE622`) |

전설마·스토리 동료가 거부된 `0x97AE29C9` 는 자격 검사(`0x2097BC0`) 쪽이고,
부엉이의 `0xD65F8D70` 은 이 두 전역 중 하나일 가능성이 높다. **게임을 켜면
전역 두 곳을 읽어 1분 만에 확정된다.**

### 4.3 엔진에 리플렉션 시스템이 있다 — 세이브 데이터의 이름·오프셋을 얻는 길

이것이 오늘의 가장 큰 소득이다. RTTI 에 이런 클래스들이 그대로 있다.

```
ReflectMetaObjectBind<MercenarySaveData>        싱글턴 전역 0x6C53B50 (게터 RVA 0x18BF230)
ReflectMetaObjectBind<MercenaryClanSaveData>    전역 0x6C53A10 (게터 0x18BF2F0)
ReflectMetaObjectBind<MercenaryClanElementSaveData>
```

속성 바인드가 클래스별로 전부 이름을 달고 있다. `MercenarySaveData` 것만:

```
CharacterKey                ← 종을 정하는 값
CharacterAppearanceIndexKey
MercenaryNo                 ← 부르기(2894)가 쓰는 번호
staticstringA               ← 이름 문자열
FactionKey · FactionNodeKey · FieldInfoKey · EquipSlotNameKey · ItemNo
float3                      ← 좌표
Vector<ItemSaveData>        ← 장비
CustomizationSaveData · ExperienceLevelSaveData
u8 · u16 · u32 · u64 · float · bool 여럿
```

그리고 명부 전체 구조가 드러난다.

```
MercenaryClanSaveData
 ├ Vector<MercenarySaveData>                    사람 용병
 ├ Vector<MercenaryClanElementSaveData>         └ 그 안에 다시 Vector<MercenarySaveData>
 ├ Vector<MercenaryClanHyosiElementSaveData>
 ├ Vector<CallMercenaryCoolTimeSaveData>        ← 소환 쿨타임이 여기 산다
 ├ Vector<CallMercenarySpawnDurationSaveData>
 └ CharacterKey · u64 · u32 · u8
```

**뜻:** 동반자 하나하나가 `CharacterKey` 를 들고 저장된다. 지금까지
"레코드 구조 미해독"이라 멈춰 있던 소유 명부(D안)가 열린다. 속성 이름은
정적으로 뽑았고, **오프셋은 게임을 켜면 메타 객체를 걸어 읽을 수 있다** —
속성 객체 하나가 약 0x110바이트이고 타입 이름 문자열(`"CharacterKey"` 등)을
생성자에서 받는다(`0x18E3100` 확인).

### 4.4 `GameData_SummonCharacter` 에도 `CharacterKey` 가 있다

`2026-09-08-companion-register-session.md` §4 는 C안(아이템 사용 소환)이
**`SummonCharacterTrackNode` 의 살아있는 인스턴스가 0개**라 막혔다고 적었다.
그 옆에 못 본 것이 있었다.

```
GameData_SummonCharacter            ← 반사 등록됨. CharacterKey + CharacterGroupKey
GimmickEventHandlerData_SummonCharacter
GameData_TimelineEvent_SummonCharacter
TrocTrGimmickExecuteSummonCharacterFrameEventReq   ← 정상 게임플레이 메시지
```

`GameData_*` 는 이 엔진에서 **적재된 에셋 객체**다. 트랙 노드와 달리 상시
메모리에 있을 가능성이 있다. 있으면 종 → 키 대응을 읽을 수 있고, 나아가
게임 자신의 소환 경로(치트가 아닌 것)를 탈 수 있다.

### 4.5 아이템 사용 타입 26종 — 미조사 후보가 남아 있다

```
ItemUseData_ConvertCharacter      ← 캐릭터를 다른 것으로 바꾼다?  미조사
ItemUseData_FeedToTarget          ← 먹이 주기(친밀도)             미조사
ItemUseData_SummonCharacterWithCatch   (조사 중, 막힘)
ItemUseData_SummonGimmickWithCatch     ← 둥지 계열로 보임         미조사
ItemUseData_RegisterReserveSlot · SealToEquip · UseSealed · RandomBox …
```

`ConvertCharacter` 는 이름만으로 판단하면 안 되지만(교훈: 이름을 믿지
말 것), **살아있는 인스턴스 수와 필드를 세는 데 5분이면 된다.**

---

## 5. 신규 방법 후보

전부 **가설**이다. 우선순위는 (되면 목표가 끝남) × (실측 비용이 쌈) × (위험이 낮음).

### N-1. 소유 명부 제자리 종 교체 ★ 최우선

**아이디어.** 무해하고 확실히 되는 종(까마귀 30029)을 2338 로 등록한 뒤,
그 `MercenarySaveData` 레코드의 `CharacterKey` 를 원하는 종으로 **제자리
한 번 쓰기**로 바꾼다.

**왜 유망한가.**
- 이 프로젝트에서 **제자리 단일 쓰기가 리로드를 넘는 것이 이미 실측**돼
  있다(인벤 소켓·스탯). 명부도 세이브 데이터 클래스이므로 같은 성질일
  개연성이 크다.
- 소환 치트를 전혀 건드리지 않는다. 월드에 개체를 띄울 필요가 없다.
- 등록 자체는 게임의 정상 경로로 이미 끝난 상태다 — 우리는 이름표만 바꾼다.

**필요한 실측(게임 켜면 순서대로, 한 시간 안).**
1. `instcount MercenarySaveData` — 살아있는 인스턴스 수가 내 동반자 수와 맞나.
2. 맞으면 **알고 있는 값으로 오프셋을 역산한다** — 등록해 둔 개체의 캐릭터
   키를 이미 아니까, 인스턴스를 덤프해 그 값이 있는 자리가 `CharacterKey` 다.
   (리플렉션 메타를 걷는 것보다 이쪽이 훨씬 싸다.)
3. 버릴 세이브에서 값 하나를 바꾸고 → 목록·소환·리로드 확인.

**위험.** 종을 바꾸면 장비·외형·스탯이 어긋나 소환 시 죽을 수 있다.
반드시 버릴 세이브. 되돌리기는 게임의 **반려동물 풀어주기**.

### N-2. 액터 종 위장 후 2338

**아이디어.** 근처의 아무 액터(까마귀)의 `[[액터+0x68]+0x20]+0x30` u16 을
원하는 종의 행 번호로 잠깐 바꾸고, 2338 을 쏘고, 즉시 되돌린다.

**근거.** §4.1 이 그 u16 이 종을 정하는 **유일한** 입력임을 보여 준다.
자격 검사도 등록 레코드도 전부 그 값을 읽는다.

**왜 2순위인가.** 살아 있는 액터의 신원 필드를 건드리는 것이라 그 액터의
AI·렌더가 프레임 중간에 어긋날 수 있다. N-1 은 이미 굳은 데이터(명부)를
건드리는 것이라 더 얌전하다. 다만 **되면 종 제한이 사실상 사라진다** —
전설마·드래곤까지 시도해 볼 수 있다(자격 검사가 여전히 거부할 수는 있다).

**필요한 실측.** 쓰기 → 2338 → 즉시 복원. 로그로 검사 함수 결과 코드 확인.

### N-3. `GameData_SummonCharacter` 경유 정상 소환

**아이디어.** §4.4 의 에셋 객체를 찾아 `CharacterKey` 를 읽고(종 대응 확보),
나아가 게임의 김믹 소환 프레임 이벤트를 태워 개체를 정상 경로로 띄운다.
띄우면 2338 이 등록한다 — 즉 금지된 2988 을 **대체**한다.

**필요한 실측.** `instcount GameData_SummonCharacter` 한 줄. 0개면 이 길도
트랙 노드와 같은 운명이고, 그때는 버린다.

### N-4. 리플렉션 워커 (범용 도구, 부수 효과가 큼)

메타 객체 전역(`0x6C53B50` 등)에서 속성 목록을 걸어 **이름 → 오프셋** 표를
찍는 probe 명령. 한 번 만들면 명부뿐 아니라 앞으로 나오는 모든 세이브 데이터
구조에 쓴다. N-1 의 2단계를 "역산" 대신 "읽기"로 바꾼다.

**비용이 더 크므로 N-1 의 역산이 실패했을 때만 만든다.**

---

## 6. 권고 순서

게임을 켜고 **버릴 세이브**로:

1. 전역 두 개(`0x6BB7AE8`, `0x6BB8A18`)를 읽어 거부 코드 이름을 확정한다 (1분)
2. `instcount MercenarySaveData GameData_SummonCharacter ItemUseData_ConvertCharacter` (5분)
3. **N-1** 오프셋 역산 → 한 마리에 단일 쓰기 → 목록·소환·리로드 (1시간)
4. 되면 오버레이에 **소유 명부 탭**(D안) + 줄마다 "종 바꾸기"를 얹는다
5. 안 되면 **N-2** 로 간다. 그것도 아니면 **N-3**

**하지 말 것:** 2988 소환 치트 되살리기(`grant.h` 경고), 3022 구동,
`trace_char_path` 재현 호출.

---

## 6.5 실측 결과 (2026-09-09, 게임 실행 중) — 거부 코드를 확정했고 해석이 틀렸다

권고 순서 1번을 실행했다. 전역 둘을 읽으니 문서가 "이름 미상"으로
남겨 둔 거부 코드가 바로 나왔다 (모듈 베이스 0x140000000).

| 전역 | 값 | 문서에 있던 이름 |
|---|---|---|
| `0x6BB7AE8` | **`0xD65F8D70`** | 부엉이가 받은 코드 |
| `0x6BB8A18` | **`0x97AE29C9`** | 전설마·스토리 동료가 받은 코드 |

### 그래서 0x97AE29C9 는 "고용 불가 유형"이 아니다

`2026-09-06` 실측표는 전설마·Damian 을 "고용 불가 유형 추정"으로
적었다. 틀렸다. 그 코드가 나오는 자리(`0x2ADE622`)의 조건을 따라가서
그 필드를 직접 읽었다.

```
[[액터 + 0x68] + 0x118]  = ClientMercenaryActorComponent  (RTTI 확인)
   +0x10 u32 상태     0x7F010001 고용됨 / 0x00010001 미고용
   +0x18 u32 고용주  0 이어야 통과. 플레이어 쪽은 0xA0100001
   +0x20 u32 번호     미고용은 0xFFFFFFFF
```

| 액터 | `+0x18` | 결과 |
|---|---|---|
| 까마귀(대조) | 0 | 통과 |
| 전설마 흑마 | `0xA0100001` | 거부 |
| Damian | `0xA0100001` | 거부 |

**사용자가 흑마를 이미 소유 중이라고 확인해 줬다.** 즉 거부 이유는
종이 아니라 **임자가 있다**는 것이다. `0xD65F8D70` 도 마찬가지로
"거리·공중"이 아니라 **핸들로 액터를 못 찾았다**는 뜻이다.

### 목록에서 구분하게 넣었다

`game/actors.{h,cpp}` 에 `actor_owner_handle()` 과 `LiveActor::owned()` 를
넣고, 근처 탭에 **소유 열**을 추가했다. 임자가 있으면 획득 버튼을
막고 이유를 툴팁으로 말한다 — 누르고 나서 코드를 보는 일이 없다.

라이브 검증(probe `nearby`, 액터 248개): 소유로 잡힌 것은 7개이고
전부 실제로 내 것이거나 파티 동료였다.

```
Animal_Black_Horse_Wild_31378  전설마 흑마
Damian                          스토리 동료
NHM_Unique_Luke_661             유니크 동료
NHM_Unique_Ronald_665           유니크 동료
Animal_Baby_Wyvern_1 × 2         부화시킨 새끼 와이번
BlackWolf                       검은 늑대
```

야생 까마귀·비둘기·앵무는 전부 미소유로 남았고, 마을 NPC
(`NHM_Unique_Andrew_647`·`Marius_468`)도 미소유로 정확히 갈렸다.

### 이것이 계획을 어떻게 바꾸나

2338 의 관문이 세 개로 확정됐고 종 제한은 그 중에 없다.

1. 액터가 월드에 있고 핸들로 찾혀야 한다 (`0xD65F8D70`)
2. 액터 종류 바이트 `[[액터+0x88]+1]` 가 4·5·6 (흑마 5, Damian 4,
   까마귀 6 — **셀 다 통과한다**)
3. 고용주가 없어야 한다 (`0x97AE29C9`)
4. 그다음 자격 검사(`0x2097BC0`)가 용병단 타입별 한도를 본다

즉 **종을 가리는 관문은 없다.** 남은 문제는 여전히 1번 하나이고,
그것이 N-1·N-2 가 노리는 자리다.

---

## 6.6 실측 결과 — N-1 과 N-3 은 원안 그대로는 죽었다 (2026-09-09)

권고 순서 2번(`instcount`)을 실행했다. 다섯 분이면 갈린다고 적었고,
실제로 그렇게 갈렸다.

| 클래스 | 살아있는 인스턴스 |
|---|---|
| `MercenarySaveData` | **0** |
| `MercenaryClanSaveData` | **0** |
| `GameData_SummonCharacter` | **0** |
| `ItemUseData_ConvertCharacter` | **0** |
| `ReflectMetaObjectBind<MercenarySaveData>` | 1 (리플렉션 싱글턴만) |

- **N-1 불가(원안).** `*SaveData` 는 저장·적재 순간에만 만들어지는
  징검다. 플레이 중에는 메모리에 없으므로 제자리 쓰기를 걸 대상이
  없다. 리플렉션으로 속성 이름을 얻은 것은 유효하나, 그것만으로는
  쓸 자리가 없다.
- **N-3 불가.** `GameData_SummonCharacter` 도 살아있지 않다 —
  `SummonCharacterTrackNode` 와 같은 운명이다. 문서가 예상한 대로고,
  한 줄로 버렸다.
- `ItemUseData_ConvertCharacter` 도 0 이라 지금 볼 것이 없다.

### 그래서 N-1 은 대상을 바꿔야 한다

런타임 명부는 세이브 구조체가 아니라 **용병단 컴포넌트**에 산다.

```
[[세션 + 0x68] + 0x110]  = (Server|Client)MercenaryClanActorComponent
```

그리고 개체마다 `ClientMercenaryActorComponent` 가 붙어 있고, 거기에
`+0x18 고용주` · `+0x20 번호` 가 있다(§6.5). **다음에 볼 곳은 용병단
컴포넌트의 번호→레코드 배열**이고, 거기에 종(CharacterKey 또는 행
번호)이 있는지를 본다. 이미 등록해 둔 개체의 키를 알고 있으므로
역산은 그대로 쓸 수 있다.

그 다음 순위는 여전히 **N-2(액터 종 위장 후 2338)** 다. §6.5 가 관문
셀을 전부 열어 두었으므로, 지금은 무엇을 만족시켜야 하는지가 명확하다.

---

## 6.7 소유 명부를 열었다 (2026-09-09, 실측)

§6.6 이 가리킨 대로 용병단 컴포넌트를 파서 런타임 명부를 찾았다.
`2026-09-05-vehicle-pet-add-review` §2.5 가 D안으로 적어 두고 "레코드
구조는 미해독" 으로 멈춰 둔 그것이다.

```
ServerMercenaryClanActorComponent   (RTTI 로 찾는다)
  +0x18  레코드 포인터 배열
  +0x20  u32 개수        +0x24 u32 용량
  +0xF8  타입별 한도 배열 (+0x100 개수) - 명부가 아니다
  +0x108 소환 중인 번호 배열 (+0x110 개수) - 여기도 아니다

레코드 0x1C0 바이트
  +0x20  u16  **캐릭터 표 행 번호 = 종**
  +0x22  u16  0xFFFF (전 레코드 공통 - 판별에 쓴다)
  +0x28  u64  MercenaryNo
  +0x50  u32  액터 핸들. 월드에 나와 있지 않으면 0
```

### 검증

레코드 22개의 행·번호가 근처 목록의 소유 액터 7개와 **1:1 로
맞았다.** 흑마 행 4074/번호 18441, Damian 행 3, Luke 행 6861,
와이번 둘 행 6608 … 각 액터의 `ClientMercenaryActorComponent +0x20`
번호가 명부 번호와 같았다.

명부 22명을 이름까지 붙여 읽었다.

| 분류 | 내용 |
|---|---|
| 전설마 3종 | 카모라(31379) · 로케이드(31378) · 로일러(31377) — 전부 보유 |
| 특수 탑승물 5 | 멧돼지·곰·아이벡스·사슴·늑대 (부적으로 얻은 것) |
| 반려동물 | 피닉스 · 새끼 와이번 ×2 · BlackWolf |
| 가축 | 양 · 염소 |
| 사람 용병 | 데미안·칼·러소·마리우스·루크·로널드 |
| 시스템 | WorldObserver (타입행 19) |

### 넣은 것

- `src/game/clan.{h,cpp}` — 컴포넌트 탐색·명부 읽기·캐시
- probe `clan` 명령 — 명부를 이름과 함께 찍는다
- 오버레이 **내 동반자** 탭 — 번호·이름·타입·행·키·월드 여부

### N-1 은 이제 대상이 생겼다

레코드 `+0x20` u16 하나를 바꾸면 이미 가진 동반자의 종이 바뀌는지가
다음 실험이다. **아직 안 해 봤다** — 살아있는 명부에 쓰는 것이라
버릴 세이브에서 해야 하고, 사용자 승인이 필요하다.

시험 순서(제안): 쓸모없는 개체(예: 양 번호 1000524, 행 3551) 하나를
골라 행을 다른 종으로 바꾸고 → 게임 목록·소환·리로드를 확인한다.
장비·외형이 어긋나면 소환 시 죽을 수 있다.

---

## 6.8 N-1 시도 — 게임을 팀겼다 (2026-09-09, 사고 기록)

사용자 승인을 받아 양(번호 1000524, 행 3551)의 종을 송아지(행 3498)로
바꿔 쓰는 시험을 했다. **게임이 팀겼고, 원인은 내 쓰기다.**

### 일어난 순서

1. 서버 레코드 `+0x20` 에 행 3498 을 썼다. `clan` 이 송아지로 읽혀다.
2. 클라이언트 사본은 그대로 3551 이었다 — **both-realms 가 여기에도
   적용된다.** 클라 쪽에도 썼다.
3. 그 사이에 **사용자가 용병 네을 더 고용해 명부가 22 → 26 이 됐다.**
   게임은 용병단 컴포넌트와 레코드를 **통째로 새로 만든다**
   (컴포넌트 주소까지 0x58172223480 → 0x581C51A9300).
4. 그것을 모르고 예전 주소에 "되돌리기" 2바이트를 썼다. 그 자리는
   이미 해제도어 **`GameData_GimmickPointData`** 가 차지하고 있었다.
5. 잠시 뒤 게임이 팀겼다. 덤프는 없고 모드 로그에도 예외 기록이 없다
   (게임 스레드에서 바로 죽은 것으로 보인다).

증명할 덤프는 없으나 정황이 명확하다 — 월드 기믹 데이터를 망가뜨렸고
그 기믹이 참조되는 순간 죽는다는 지연 크래시 모양이 맞아떨어진다.

**세이브는 안전하다.** 디스크에 쓴 것은 없고, `GameData_*` 는 세이브가
아니라 적재된 에셋이며, 명부의 종 변경도 재구성 때 원래대로 돌아갔다.

### 배운 것

**하나. 명부 주소는 휘발성이다.** 동반자가 하나만 늘어도 컴포넌트부터
새로 만들어진다. 읽은 주소는 다음 순간 남의 것이다.

**둘. 살아있는 게임에 쓸 때는 쓰기 직전에 다시 찾고 표식을 확인한다.**
레코드는 `+0x22 == 0xFFFF` 와 `+0x28 == 번호` 로 스스로를 증명할 수 있는데
그것을 안 보고 썼다.

**셋. 생 `poke` 로 구조체에 쓰지 말 것.** 검증을 강제하는 명령을 따로
두어야 한다.

### 그래서 넣은 안전장치

- `resolve_species_write(rtti, reader, 번호, &대상)` — 부를 때마다 클라·서버
  양쪽을 **번호로 다시 찾고** 표식까지 확인해 주소를 낸다. 주소를
  들고 있다가 쓰는 일이 구조적으로 불가능해진다.
- probe `setspecies <번호> <행>` — 해석 → 검증 → 양쪽 쓰기 → **다시
  해석해 확인**까지 한 번에 한다. 명부에 쓸 때는 이것만 쓴다.
- `refresh_clan_roster` 가 캐시해 둔 컴포넌트가 아직 컴포넌트 꼴인지 먼저 본다.

### 아직 모르는 것

종을 바꾸면 **게임 UI 와 소환이 그것을 따르는가** 는 확인하지 못했다.
바꿔 둔 상태에서 명부가 재구성되면서 변경이 사라졌기 때문이다.
다음에는 `setspecies` 로 바꾸고 **즉시** 화면을 확인한다.

---

## 6.9 N-1 성공 — 종을 바꾸는 것이 화면으로 확인됐다 (2026-09-09)

안전장치(§6.8)를 넣고 다시 했다. **된다.**

```
세이브 백업: %LOCALAPPDATA%\Pearl Abyss\CD\save
        -> save_backup_2026-09-09_setspecies (21MB)

setspecies 1000414 3436
  번호 1000414: 서버 0x33E96301520(행 6798) · 클라 0x33F1FFF11A0(행 6798) -> 행 3436
  확인: 서버 행 3436, 클라 행 3436 - 반영됨
```

대상: `Riding_Warthog_1000`(바위엄니 혹멧돼지, 타입행 5)
새 종: `Animal_Lion_Domestic_30374`(행 3436, 타입행 5, 표시명 ‘사자’)

**사용자가 게임 특수 탑승물 목록에서 ‘사자’로 바뀜 것을 확인했다.**

### 어떤 종을 고를것인가

- **같은 용병 타입(행)을 고른다.** 특수 탑승물은 타입행 5. 타입을 넘으면
  용병단의 타입별 한도(컴포넌트 +0xF8)와 어긋날 수 있다 — 아직 안 해 봤다.
- **`_Domestic` 변형을 고른다.** 소유판이다. `_Wild` 는 야생 개체고
  `Riding_*` 는 탑승 상태 변형이다. 호랑이·사자는 `Riding_` 변형이
  아예 없고 `Animal_Lion_Domestic_30374` 가 소유판이다.
- 행 번호는 probe `charfind <이름조각>` 으로 찾는다. **키가 아니라 행이다.**

### 아직 남은 확인

1. 소환하면 실제로 사자 모델이 나오고 타지는가
2. 저장·리로드를 넘는가 (인벤 제자리 쓰기는 넘었다 - [[inventory-inplace-write]])
3. 타입을 넘는 교체(예: 타입행 11 가축 -> 타입행 1 말)가 되는가

---

## 6.10 결론 — 종 교체가 답이다 (2026-09-09, 전 과정 검증)

사용자 확인으로 네 단계가 전부 통과했다.

| 단계 | 결과 |
|---|---|
| 명부 레코드 `+0x20` 제자리 쓰기 (클라+서버) | 됨 |
| 게임 특수 탑승물 목록에 반영 | 됨 - 사자로 바뀜 |
| 소환하면 사자가 나오고 탑승 | 됨 |
| 저장·리로드 생존 | 됨 (용병단 주소가 바뀌었는데도 행 3436 유지) |

즉 **원하는 종을 얻는 문제는 풀렸다.** 부적 6종도, 포획도, 금지된
소환 치트도 필요 없다. 이미 가진 동반자 하나를 골라 종만 바꾸면 된다.

### 쓰는 법

```
probe:     charfind <이름조각>        새 종의 **행 번호**를 찾는다
           clan                        내 동반자의 **용병번호**를 찾는다
           setspecies <번호> <행>     바꿔 쓴다(검증·양쪽·재확인 포함)
오버레이:  내 동반자 탭 → 줄마다 [바꾸기] → 종을 고른다
```

고를 때의 규칙: **같은 동반자 타입**(특수 탑승물은 타입행 5), 그리고
`_Domestic` 같은 **소유판 변형**을 고른다. `_Wild` 는 야생 개체,
`Riding_*` 는 탑승 상태 변형이다. 호랑이·사자는 `Riding_` 변형이 없고
`Animal_Lion_Domestic_30374`(행 3436)가 소유판이다.

### 아직 안 해 본 것

- **타입을 넘는 교체**(가축 -> 말 등). 용병단의 타입별 한도(+0xF8)와
  어긋날 수 있다. 오버레이는 기본으로 같은 타입만 보여 주고, 체크를
  풀면 경고를 붙인다.
- 드래공·ATAG 등 포획 대상이 없는 종으로의 교체. 타입이 다르므로 위와
  같은 문제다.

---

## 6.11 종을 고를 때의 진짜 기준 — 탈것 표 연결 (2026-09-09)

사용자가 성체 와이번으로 바꿠 동반자가 탈것·특수·반려동물 어디에도,
캐릭터 선택창에도 안 보인다고 짚었다.

### 타입이 아니라 필드 하나다

처음엔 "타입행 6 은 게임이 안 보여 준다" 로 적었다. 틀렸다.
같은 타입 6 인 낙타로 바꿔 봐도 안 보였지만, 그것은 타입 탓이 아니었다.

`CharacterInfo` 레코드의 **`+0x16A` u16** 이 갈람한다. 0xFFFF 면 없다.

| 종 | 행 | 타입행 | `+0x16A` | 게임 표시 |
|---|---|---|---|---|
| `Animal_Lion_Domestic_30374` | 3436 | 5 | **9** | 보임 |
| `Animal_Baby_Wyvern_1` | 6608 | 9 | **10** | 보임 |
| `Riding_Wyvern_1000` | 6804 | 5 | **9** | 보임 |
| `Animal_Camel_Domestic_30106` | 3583 | 6 | **0xFFFF** | 안 보임 |
| `Animal_Wyvern_30508` | 4214 | 6 | 0xFFFF | 안 보임 |

값 9·10 은 **탈것(VehicleInfo) 표의 행**으로 보인다(표는 34행). 그 연결이
있어야 탑승물로 등록·표시된다.

### 성장한 와이번은 다른 행이다

사용자 설명: 새끼 와이번을 키우면 탈 수 있는 짐승 탈것이 된다.
그 결과물은 `Animal_Wyvern_30508`(타입 6, 연결 없음)이 아니라
**`Riding_Wyvern_1000`(행 6804, 타입행 5, 연결 9)** 다. 이미 가진
`Riding_Bear_1001`·`Riding_Wolf_1001` 과 같은 계열이다.

그로 바꾸자 **목록에 떴다.** 표시 문제는 여기서 풀렸다.

### 남은 것 — 소환 시 팀김

목록에서 고르자 게임이 팀겼다. 로그에 2894 가 나가고 그 뒤
"소환 작업 -> 코드" 가 없다 - 소환 도중에 죽었다.

짚히는 차이: 그 둘(1000607·1000608)은 **야생 성체 와이번을 2338 로
획득한 것**이라 레코드 `+0x30~+0x4F` 에 야생 개체의 값(float 여럿)이
남아 있다. 정상 동작한 사자(1000414)는 그 구간이 **전부 0** 이었고,
그것은 부적 경로로 들어온 레코드다.

다음 시험(싸고 안전): 부적로 얻은 `Riding_Bear_1001`(번호 1000415)를
`Riding_Wyvern_1000` 으로 바꿠다. 깨끗한 레코드면 되는지가 갈린다.

---

## 6.12 마무리 — 게임에서 전부 확인됨 (2026-09-09)

마지막 배포 뒤 사용자가 게임에서 확인했고, 로그로도 받침된다.

```
16:37:42  아이템 대응표: 6813개 (키 -> 순번)        시작 21초 만에
16:46:07  획득 응답: 새 동반자 번호 1000601
16:46:10  획득 뒤처리: 번호 1000601 의 소환 판정(핸들 0xB0100189)을 풀었다
16:47:48~57  소환 작업: 1000605·1000603·1000602·1000601·1000595·1000594
             여섯 마리 연속, 전부 코드 0 (성공)
```

앞서 먹통이던 개체들이 전부 풀렸다. 지역 이동도 세이브/로드도 없이
획득 3초 뒤 바로 쓸 수 있다. 구동 멈춤은 0회였고 구동 자리는 매번
안전한 `+2A0263D` 였다.

### 오늘 세운 규칙 세 가지

1. **종을 고를 때는 `CharacterInfo +0x16A`(탈것 표 연결)가 있는 것만.**
   0xFFFF 면 게임 목록 어디에도 안 뜼다.
2. **바꾸기는 깨끗한 레코드에만.** 근처 획득분은 레코드 +0x30~+0x4F 에
   야생 개체 값이 남아 종을 바꾸면 소환이 게임을 죽인다.
3. **구동은 확인된 자리에서만.** 그 밖의 자리에서 부르면 게임 로직
   스레드가 통째로 멈춰 저장도 종료도 안 된다.

### 남은 것

내 동반자 목록에 스토리 NPC 와 그들의 탈것(예:
`Riding_Horse_Tiuta_Unique_2050_Demian`)이 섞인다. 레코드 +0x24 를 소유
구분 후보로 봤으나 값이 "_slo"(ASCII) 같은 쓰레기였다 - 아니다.
다음 단서는 리플렉션의 `MercenaryClanElementSaveData` 다. 명부가 그룹으로
나뉘고 평면 배열(+0x18)이 그 합집합일 가능성.

---

## 6.13 정정 — 그 필드는 +0x6A 이고, 단독으로 표시를 정하지 않는다

§6.11 을 두 군데 고친다. 둘 다 내 오류다.

**하나. 오프셋이 `+0x16A` 가 아니라 `+0x6A` 다.**
덤프 주소를 "레코드+0x160" 으로 적었는데 실제로는 레코드+0x60 이었다.
0x16A 로 읽으면 전부 257(0x0101) 이 나온다 - 엉뚜한 자리다.
`+0x6A` 는 2338 고용 함수가 `movzx ebx,[rax+0x6a]` 로 읽는 바로 그 자리다.

**둘. 성체 와이번은 0xFFFF 가 아니다.**
표에 0xFFFF 로 적었는데 재지 않고 낙타와 같겠거니 넘겨짚은 것이다.
재보니 **9** 였다.

### 문 값의 실제 모양

| 타입행 | `+0x6A` |
|---|---|
| 5 특수 탑승물 | 9 |
| 9 반려동물 | 10 |
| 6 짐승 탈것 | 와이번 9, 낙타 65535 |
| 동반자 아님 | 65535 |

타입을 거의 그대로 따라가는 분류 값이다. **이것만으로 표시 여부가
갈리지는 않는다** - 성체 와이번은 9 를 가졌는데도 게임 목록에 안 떴다.
65535 는 분명히 안 뜼니(낙타 실측) 걸러내는 데만 쓴다.

### 바꾸기에 와이번이 안 보였던 진짜 이유

- **성체 와이번**(타입행 6): 사용자가 요청한 3종 필터(1·5·9)에 안 들어간다.
  그리고 실제로 그것으로 바꾸면 게임에 안 떴다 - 막는 것이 맞다.
- **새끼 와이번**(타입행 9): 팝업의 ‘같은 타입만’ 기본 체크 때문이다.
  특수 탑승물에서 열면 반려동물이 안 보인다.

교차 타입 교체가 이미 검증됐으므로 **그 체크를 기본 해제로** 바꾸고,
후보 개수를 팝업에 찍어 거르기가 뭐를 숨겼는지 보이게 했다.

---

## 6.14 "야생 획득분" 표시는 틀렸다 (실측 2026-09-09 18:30)

명부 레코드 8개를 통째로 떠서 `+0x30~+0x4F` 를 직접 봤다. 이 구간이
0 이 아니면 야생 획득분이라고 판정하고 바꾸기를 막고 있었는데,
**출신과 아무 상관이 없다.**

| 레코드 | 출신 | `+0x30~+0x4F` | 옛 판정 |
|---|---|---|---|
| 아이벡스 1000416 | 부적 | 전부 0 | (없음) ✔ |
| 피닉스 1000419 | 부적 | 전부 0 | (없음) ✔ |
| 새끼 와이번 1000481 | - | 전부 0 | (없음) |
| **사자 1000414** | 종 교체분 | float + 0xFF 채움 | **야생획득 ✘** |
| **카모라 18442** | 전설마 | `+0x4C` = 0xFFFFFFFF | **야생획득 ✘** |
| **대형늑대 1000599** | 야생 | 전부 0 | **(없음) ✘** |
| 흰날개딱새 1000594 | 야생 | 힙 포인터 4개 | 야생획득 |
| 사막여우 1000595 | 야생 | 힙 포인터 4개 | 야생획득 |

한 필드가 아니라 **여러 필드가 겹쳐 있는 구간**이라 "아무 바이트나
0 이 아니면" 이라는 판정이 세 가지 서로 다른 것을 하나로 뭉갰다.
막아야 할 것(까마귀·대형늑대)은 못 막고 멀쩡한 것(사자·카모라)만
막고 있었다. 제거했다.

`+0xA0` 도 출신 표시가 아니다. 아이벡스 400 · 사자 450 · 곰 1500 만
값이 있고 **부적으로 얻은 피닉스를 포함해** 나머지는 전부 0xFFFFFFFF다.

## 6.15 크래시가 난 자리를 특정했다

로그 보관(`.prev`) 덕에 죽은 판의 기록이 남았다. `CDToybox.log.prev2`
마지막 세 줄:

```
18:02:40  소환 1000415 곰       -> 코드 0 (성공)
18:02:43  소환 1000599 대형늑대 -> 코드 0 (성공)
18:02:45  동반자 캡처 [스폰선택] ID 2894 ... 9A 44 0F 00 ...   <- 1000602 까마귀
          (결과 줄 없음 - 게임의 소환 작업 함수 안에서 죽었다)
```

**까마귀(1000602) 하나가 죽였다.** 그런데 다시 켠 뒤 같은 레코드를
형제들(앵무새 1000601 · 멧새 1000603 · 앵무새 1000605)과 바이트로
비교하면 **행 번호·번호·좌표·할당 주소를 빼고 전부 같다**. 지금은
월드에 정상으로 나와 있다.

그러면 죽인 것은 레코드의 영구 상태가 아니라 **종을 바꾼 그 판에서만
생기는 무언가**다. 저장하고 다시 켜면 사라진다. 야생 출신이라서가
아니다 - 같은 판에서 부적 출신 곰을 와이번으로 바꿔 소환한 것은 됐다.

## 6.16 크래시 로거 (`src/core/crashlog.cpp`)

여기까지 오는 동안 크래시가 날 때마다 남는 것이 "로그가 끊긴 자리"
뿐이었다. 그래서 예외 핸들러를 넣었다.

- 치명적 예외(접근 위반·0 나누기·잘못된 명령 등)만 걸러 `bin64/CDToybox.crash.txt`
  에 **덧붙여** 쓴다. 최대 16회.
- **CRT 를 한 줄도 쓰지 않는다.** `CreateFileW`/`WriteFile` 뿐이다.
  힙이 깨진 채 죽으면 `std::format`/`ofstream` 이 힙 락에서 멈추고,
  그러면 게임이 죽는 대신 굳어 강제 종료해야 한다.
- 남기는 것: 예외 코드·RIP(모듈+RVA)·접근하려던 주소·주요 레지스터,
  그리고 RSP 부터 0x800 을 훑어 **게임 모듈 안을 가리키는 값**을
  복귀 주소 후보로 20개까지.
- 예외를 처리하지 않고 `EXCEPTION_CONTINUE_SEARCH` 로 흘려보내므로
  게임의 원래 동작은 그대로다.

다음에 소환이 죽으면 어느 명령에서 무엇을 만졌는지 나온다.

---

## 6.17 종 교체 크래시의 진짜 원인과 수정 (해결 2026-09-09 20:00)

크래시 로거가 자리를 찍었다(`CDToybox.crash.txt` 19:35:35):
`모듈+0x2AD520F` 의 `cmp qword ptr [rax+0x28], -1` 에서 **주소 0x28** 을
읽다 죽었다 = rax 가 0. 바로 앞이 `call 0x208F5C0`(용병번호 -> 레코드
해시 조회)이고, 못 찾으면 0 을 낸다.

넘어가는 번호의 출처는 호출자 `0x2ABD850` 에 그대로 있다.

```
0x2ABD8C0  mov rdi, -1                  기본값
0x2ABD98A  call 0x2095600               종류로 등록된 동반자 찾기
0x2ABD9A2  cmovne rdi, [찾은것+0x28]     못 찾으면 -1 그대로
0x2ABDBB9  call 0x2AD4F70(.., rdi, ..)  -1 을 그대로 넘긴다
```

`0x2095600` 은 종류(=`CharacterInfo +0xBE`)별 색인(`0x208FC30`)을 뒤지고,
없으면 전역 **빈 레코드**(`0x6BB3F70`)를 낸다. 명부 레코드의 종(+0x20)을
제자리에서 바꾸면 그 색인이 안 따라오므로 조회가 빈다.

**게임 자신이 같은 조회를 쓰는 다른 두 자리에는 널 검사가 있다:**

```
0x2ABD8EB  call 0x208F5C0     0x2AD531D  call 0x208F5C0
0x2ABD8F0  lea rbx,[0x6BB3F70] 0x2AD5322  lea rcx,[0x6BB3F70]
0x2ABD8FA  cmovne rbx, rax     0x2AD532C  cmovne rcx, rax
```

죽는 `0x2AD51F8` 한 자리에만 빠져 있다. **게임 버그다.**

수정(`src/game/spawnguard.cpp`): 그 call 하나를 썽크로 돌려 널이면 빈
레코드를 돌려준다. 조회 함수 자체는 안 건드리므로 다른 호출자는 그대로다.
설치 전에 (1) 그 자리가 그 call 인지 (2) 빈 레코드가 `+0x28 == -1`,
`+0x20 == 0xFFFF` 로 초기화됐는지 확인한다.

**결과 (사용자 확인 2026-09-09 20:00):**

| 대상 | 바꾼 종 | 결과 |
|---|---|---|
| 까마귀 1000602 (야생 획득) | `Riding_Wolf_1001` 행 6799 | 소환·탑승 성공 |
| 회색 앵무새 1000605 (야생 획득) | `Riding_Wyvern_1000` 행 6804 | 소환·탑승 성공 |

**야생 획득분도 종을 바꿔 탈 수 있다.** 반려동물(타입행 9)에서 특수
탑승물(타입행 5)로 넘기는 것도 된다.

### 곁다리 정정: 소환 코드 0 이 아니라고 실패가 아니다

두 번 다 `소환 작업 ... -> 코드 0xFAABC030` 이 찍혔는데 **실제로는
성공**이었다. 그 값은 게임 안 121곳이 쓰는 범용 코드(전역 RVA
`0x6BB7EB8`)다. 로그가 그것을 "거부" 라고 단정하고 있었다 - 16진수만
남기고 뜻을 붙이지 않도록 고쳤다.

---

## 6.18 소유자 필드와 표시 관문 (실측 2026-09-09 20:50)

### 소유자는 레코드 +0x148 이다

게임 자신의 판정 함수 `0x209FE40(레코드, 행)` 에서 나왔다. 종(+0x20)으로
CharacterInfo 를 찾아 `_mercenaryInfo`(+0xBE) -> MercenaryInfo 의
`_summonOwnerOption`(+0x5D) 분류를 보고, 분류 0~3 중 어느 쪽이든 결국
**+0x148 u16 을 비교**한다.

| 분류 | 비교 |
|---|---|
| 0 | 무조건 거짓 |
| 1 | 넘긴 행 == 주인공 행, 그다음 +0x148 == 주인공 행 |
| 2 | +0x148 == 주인공 행 |
| 3 | +0x148 == 넘긴 행 |

명부에서 확인: `1000109 브리안토 [데미안]` - 데미안의 말이다. 주인공
소유는 `[클리프]` 로 찍힌다. 나머지는 0xFFFF(기록 없음)다. 스토리 용병
(칼·루소·루크)은 0xFFFF 라 이 값만으로 전부 갈리지는 않는다.

### 표시 관문은 _equipInfo 가 아니라 **동반자 타입**이었다

`tools/rtti/fields.py` 로 이름을 확정했다: `+0x6A` = `_equipInfo`,
`+0xBE` = `_mercenaryInfo`. 그리고 실측:

| 종 | 행 | 타입행 | _equipInfo | 셈 | 게임에 뜨나 |
|---|---|---|---|---|---|
| 낙타(Domestic) | 3583 | **6** | 65535 | 0 | 안 뜸 |
| 성체 와이번 | 4214 | **6** | **9** | 0 | 안 뜸 |
| 새끼 와이번 | 6608 | 9 | 10 | 1 | 뜸 |
| Riding_Wyvern_1000 | 6804 | 5 | 9 | 1 | 뜸 |
| Riding_Wolf_1001 | 6799 | 5 | 9 | **0** | 뜸 |

**성체 와이번은 `_equipInfo` 가 9 인데도 안 뜬다.** `Riding_Wolf_1001` 은
셈이 0 인데도 탄다. 둘 다 관문이 아니다. 안 뜨는 이유는 타입행 6
(Vehicle) 이고, 그건 사용자가 요청한 1/5/9 필터가 이미 거른다.

전수 집계(`probe gate`): 타입행 1·9 는 `_equipInfo` 없는 것이 0건,
타입행 5 는 63중 4건. 그 4건은 이것뿐이다.

```
행 6703  Riding_Balloon_Summoner_1   열기구
행 6704  Riding_Balloon_Summoner_2   구름길 열기구
행 6705  Riding_Balloon_Summoner_3   하늘길 열기구
행 7038  Animal_Tiger_Wild_2         호랑이
```

숨길 이유가 없어 조건을 뺐다. 소환 널 가드(6.17)가 있어 잘못 골라도
게임이 죽지 않으므로 시험이 안전해졌다. 팝업에 `장비` 값을 찍어 어떤
것이 그 4종인지 보이게 했다.

### 종 교체가 저장·재시작을 넘었다

`1000602 -> Riding_Wolf_1001`, `1000605 -> Riding_Wyvern_1000` 둘 다
리로드 뒤 그대로였다.

---

## 6.19 플레이어블 캐릭터 구분 · _equipInfo 의 진짜 뜻 (2026-09-09 21:30)

### 플레이어블 캐릭터는 데이터로 갈린다

두 갈래다. 이름을 박지 않으므로 DLC·모드로 늘어도 따라간다.

1. **주인공 클리프**는 용병 표에 없다 - 캐릭터 행 0 `Kliff`,
   `_mercenaryInfo` 0xFFFF. 게임이 소유자 판정(0x209FE40)에서 쓰는
   사슬로 **지금 조종 중인 캐릭터 행**을 읽는다. 실측값 0.

   ```
   전역 0x6C29AF8 (또는 0x6C29B00) -> [+0] -> [+8] -> [+0x28] -> +0x100 u16
   ```

2. **바꿔 탈 수 있는 나머지**는 `MercenaryInfo._mercenaryType == 1`
   (Mercenary_Main)이다. 지금 6행뿐이다.

   | 캐릭터 행 | 내부 이름 | 표시명 |
   |---|---|---|
   | 3 | Damian | 데미안 |
   | 5 | Oongka | 웅카 |
   | 6 | Yann | 얀 |
   | 7 | Yahn | 얀 |
   | 8 | Nairah | 나이라 |
   | 9 | Witch | 마녀 |

**`_isPlayable`(MercenaryInfo +0x22)은 이 용도로 못 쓴다.** 실측하니
Mercenary_Main 뿐 아니라 Vehicle_Horse·Vehicle_Dragon·
Vehicle_WarMachine·Vehicle_Special·Vehicle 까지 전부 1이다
(Vehicle_Ship·펫·가축·용병은 0). 뜻은 "플레이어가 조종하는가" 에 가깝다.

### `_equipInfo` 는 표시 관문이 아니라 **탑승 가능 여부**였다

6.18 에서 조건을 뺐더니 호랑이가 목록에 돌아왔고, 사용자가 시험했다.

| 종 | 행 | 타입행 | _equipInfo | 결과 |
|---|---|---|---|---|
| `Animal_Tiger_Wild_1` | 3438 | 5 | 9 | **탑승 O** |
| `Animal_Tiger_Wild_2` | 7038 | 5 | 65535 | 소환 O, **탑승 X** |

그래서 숨기는 대신 팝업에 **탑승 가능/불가** 로 찍는다. 고르는 사람이
알고 고르면 된다.

주의: "안 움직인다"는 근거가 아니다 - 탈것은 타지 않으면 원래 움직이지
않는다(사용자 지적). 실측된 것은 탑승 불가 하나다.

### 소유자 컬럼

레코드 +0x148 로 푼 소유자가 플레이어블이면 그대로, NPC 소유면 흐리게
찍는다. 판정은 위 두 갈래를 쓴다.

---

## 7. 근거 파일

- 정적 분석 이번 회차: `tools/rtti/disasm.py` · `find_class.py` 로 수행.
  범위 디스어셈블러와 리플렉션 탐색은 임시 스크립트였다 — 다시 쓸 것 같으면
  `tools/rtti/` 로 승격할 것.
- 기존 문서: `2026-09-04-vehicle-pet-review` / `2026-09-05-vehicle-pet-add-review`
  / `2026-09-05-catchable-companions` / `2026-09-05-companion-summon-acquire-design`
  / `2026-09-05-virtual-amulet-review` / `2026-09-05-session-companion-handoff`
  / `2026-09-06-session-close` / `2026-09-07-companion-add-session`
  / `2026-09-08-companion-register-session`
- 코드: `src/game/companion.{h,cpp}` · `roster.{h,cpp}` · `actors.{h,cpp}`
  · `src/render/roster_panel.cpp`
