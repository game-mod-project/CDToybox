...
```
0. [읽기] Knowledge_CallVehicle(행 5024) · Knowledge_CallDragon(행 5025) 의
   현재 레벨과, 서버 해시맵에 그 둘이 들어 있는지를 읽는다.
   - 대조군: 말은 잘 불리므로 CallVehicle 쪽은 맵에 있어야 정상이다
   - 이것만으로도 가설이 반쯤 갈린다. **쓰기가 없다**
1. [배관] 0x02AA55D0 호출을 작업 디스패처(RVA 0x13A6750)에 건다.
   렌더 스레드에서 부르지 않는다(TROUBLESHOOTING 1.13)
2. [쓰기] 그 경로로 Knowledge_CallDragon(키 1000175, 레벨 1)을 넣는다
3. 게임에서 저장 → 7시 슬롯에서 드래곤을 누른다
4. **모션이 나오는가**를 본다. 로그가 아니라 화면으로 본다 -
   모션 유무는 우리 훅에 안 잡힌다(TROUBLESHOOTING 4.29 의 교훈)
```

**0번이 가장 값싸고 가장 많이 가른다.** 서버 해시맵을 읽기만 해도
"드래곤 지식이 없어서인가" 가 확인된다. 쓰기는 그 뒤에 한다.
