...
> **정리 (2026-09-21) — ✅ 해결.** 드래곤은 이 문서의 단서로 풀렸다 — 소환 · 탑승 ·
> 비행 게임 확인(2026-09-16, `STATUS.md` §1.21). 머리말의 "라이브 검증은 아직 안 했다"
> 와 해결 전 본문은 그대로 두었다. **"레벨 표를 쓰는 것으로는 안 된다" 는 취소** —
> 레벨 표 쓰기로 충분하다(스킬트리 지식에만 참인 말, `STATUS.md` §1.21).
> failmessageinfo 는 이후 전부 풀렸다(`2026-09-16-vehicle-place-and-dismount.md`). "등록
> 항목을 바꿔 부르는 길은 다시 파지 말 것" 은 좁혀졌다 — 같은 카테고리 안에서는 된다
...
### 0-0-3. 끝 — **타고 난다** (사용자 확인)

소환에서 멈추지 않는다. 드래곤에 **탑승해 비행까지 된다.** 이 건은 닫는다.

### 0-0-2. **블랙스타 레코드는 둘이고, 휠에 올라가 있던 쪽이 빈 껍데기였다**

(2026-09-16 저녁, 실측 · 재시작으로 확인)

명부에 `Riding_Dragon_1`(행 7008) 레코드가 **둘** 있다. 앞선 문서가 이 둘을
"realm 마다 한 벌" 로 읽었는데 **틀렸다** — `clan client` 목록에 둘 다 나온다.
**서로 다른 동반자 둘**이다.

| 칸 | 1000483 | **1000724** |
|---|---|---|
| `+0xA0` 생명 | **1** | **2500** (A.T.A.G. 와 같은 값) |
| `+0x158`/`+0x15C` 성장치 | **1 / 1** | **30 / 30** (사자와 같은 값) |
| `+0x148` 휠 칸 | `0x0000` (**등록돼 있었다**) | `0xFFFF` |

소환 경로가 집던 것이 **스탯이 빈 1000483** 이었다:

```
18:04:27.600  등록조회(0x2096C30): 카테고리=2 -> 찾음 (+0x20=7008 +0x28=1000483)
```

**등록을 1000724 로 옮기자**(`+0x148` 을 서로 바꿔 씀) **완전 종료·재시작 뒤
스탯이 2500 으로 떴다**(사용자 확인). 그 쓰기는 세이브에 남는다.

```
poke <1000724>+0x148  0000     올린다
poke <1000483>+0x148  FFFF     내린다
```

**어떻게 찾았나 — 같은 종 두 레코드를 맞댔다.** 드래곤을 A.T.A.G.·말·사자와
비교하는 동안은 차이가 대부분 좌표라 안 보였다. 같은 종끼리 대니 스탯 자리가
한 번에 드러났다.
...
| 후보 | 결과 |
|---|---|
| 정적 `characterinfo` 의 스탯 배선 | 드래곤 vs A.T.A.G. vs 말 **`recdiff` 차이 0건** |
| `_grownLevel`(+0x498) · `_statusGroupInfo`(+0x5B8) | 셋이 동일 |
| 장비(파트) | 사용자 확인 — 말·A.T.A.G.도 **장비가 체력에 영향 없음** |
| `MercenarySaveData` 반사 속성 | 스탯 속성이 **없다** |
| `+0xA0` 에 직접 쓰기 | **게임이 곧바로 되돌린다**(1 로 복구됨) - 게임이 관리하는 칸 |

### 0-0-1. 남은 것 — **스탯이 비어 있다** (아래는 위 §0-0-2 로 풀렸다)
