...
// 창 안에서 움직여도 창에 WM_MOUSEMOVE 가 안 들어온다. 백엔드(imgui_impl_win32)는
// WM_MOUSEMOVE 를 한 번이라도 받아 추적 중(MouseTrackedArea != 0)이면 GetCursorPos 대체
// 경로를 끄므로, 그 상태로 마우스룩에서 열면 소프트 커서가 그 자리에 굳는다(진단 로그:
// "OS 커서는 창 안에서 움직였는데 ImGui 좌표가 멎었다", 포그라운드 true).
//
...
//   창 메시지가 끊긴 것이 실측되면(filter.h legacy_gate_step) raw input 에서 모아 둔
//   버튼·휠도 넣는다. 창 메시지가 살아 있으면 raw 쪽은 버린다(둘 다 넣으면 휠이 두 번 간다).
