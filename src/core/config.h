...
struct Config {
    ...
    // 탈것 **호출 검증기**(RVA 0x9DD420) 진단 훅. **기본은 끔.**
    //
    // 2026-09-21: 이것을 상시로 걸었다. 주석에 *"형제들과 같은 순수 통과
    // 훅이니 괜찮다"* 고 적어 놓고 근거 없이 예외를 만든 것이다. 바로 위
    // `summon_diag` 가 **꺼져 있는 이유**가 그대로 여기에도 걸린다 - 조사용
    // 훅은 기능이 아니고, 게임 함수 한복판에 상시로 앉을 이유가 없다.
    // ini 표기: `call_diag = 1`
    bool call_diag = false;

    // **보스룸에서도 탈것 호출**(`game/bosscall.h`). 시작할 때 켠다. 화면의
    // 토글과 같은 것이다 - 보스룸 같은 곳이 거는 행동 제한(허용 스킬그룹 목록 ·
    // 탑승 금지)을 **호출 스킬 판정과 호출 검증기에서만** 넘긴다. 기본은 끔.
    // ini 표기: `boss_call = 1`
    bool boss_call = false;
