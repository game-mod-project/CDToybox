...
void auto_analysis_loop() {
    ...
    while (!g_stop.load()) {
        ...
        // **수배 컴포넌트도 똑같다.** 위 문단과 같은 이유인데 이쪽만 빠져 있었다 -
        // 벌금 사슬을 찾는 루프(2번)는 **카메라를 잡으면 break 로 끝난다.** 카메라는
        // 타이틀 화면에서도 잡히므로, 월드에 들어가기 전에 루프가 끝나 버리고
        // `wanted_component_find` 는 그 뒤로 영영 안 불렸다. 화면에는 "월드에
        // 들어가면 저절로 잡습니다" 가 영원히 남는다(사용자 보고 2026-09-18).
        //
        // 2026-09-18 실측: 힙에 `ClientSelfWantedActorComponent` 가 멀쩡히 있는데
        // (0x27714C023E0) 로그에 후보 줄조차 없었다 - 탐색 루프에 **도달을 못 했다**.
        // 안에서 월드 게이트 + 15초 스로틀로 스스로를 막으므로 여기서 매 바퀴
        // 불러도 값싸다(명부와 같은 규칙).
        wanted_component_find(rtti, reader);
