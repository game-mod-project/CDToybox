...
// 지급 가능한 아이템 키의 상한.
//
// 실측 2026-09-06 (아이템 표 6,813개 전량 덤프 분석): 아이템 표의 키는 곧
// record+0x00 이며(6,813개 전부 일치, 오독 0), 대부분은 ~2.6M 이하의 작은
// 키다. 그런데 94개가 10M 이상의 큰/해시형 키를 쓴다 - 오독이나 쓰레기가
// 아니라 진짜 게임 키다(유효한 이름·분류·등급을 가진 실제 아이템). 판금
// 갑옷 세트·장신구(귀걸이·표식·반지)·도구(곡괭이·도끼)·안장 등 키를 큰
// 값/해시로 매긴 별도 콘텐츠 클래스로 보인다.
//
// 문제는 치트 지급 경로(CreateItemFromTrItemValue)가 이 아이템들을
// 재생성하지 못한다는 것. 지급하면 게임 생성자는 통과시키지만("지급 끝")
// 깨진 아이템이 인벤토리에 쌓여 잠시 뒤 크래시했다. 정상 최대(2.6M)와
// 문제군 최소(10.02M) 사이가 완전히 비어 10M 로 자르면 깔끔하게 갈린다.
// 부르는 쪽(지급 큐)에서 미리 거른다. 이 94개는 세이브 간 이월 불가.
