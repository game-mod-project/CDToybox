...
// ----------------------------------------------------------------------
// 종을 골라 동반자로 등록 (소환 -> 획득 두 걸음)
//
// **실측으로 끝까지 확인된 사슬이다**(2026-09-07).
//
//   캐릭터 표에서 종 선택
//     -> 소환 치트 2988 (플래그 kSpawnCharKind) 로 월드에 띄운다
//     -> 그 액터 핸들로 획득 2338
//     -> 게임이 획득응답(2107)에 용병번호를 실어 보낸다
//     -> 모드의 동반자 탭에 나온다
//
// 양(30026)은 번호 1000524, 염소(30027)는 1000525 를 받았다. 둘 다
// 가축이라 게임의 반려동물 UI 에는 안 뜨지만 명부에는 올라간다.
//
// 두 걸음인 이유: 소환은 대기열을 거쳐 게임 스레드에서 실행되고, 액터가
// 목록에 잡히기까지 몇 초 걸린다. 그래서 시작만 걸어 두고 매 틱마다
// 새로 생긴 액터를 찾아 획득한다.
//
// 되는 종과 안 되는 종이 있다. 캐릭터 표에 고용 예여야 하고, 게임의
// 소환 표에도 있어야 한다. 말(32960)은 표를 통과하고도 액터가 나오지
// 않았다 - 종별 차이는 아직 모른다. 적대 상태로 나오는 종(곰)은 획득이
// 거부된다.
bool companion_register_start(std::uint32_t char_key);
// 매 프레임 부른다. 아무것도 안 걸려 있으면 즉시 돌아온다.
void companion_register_tick(const mem::Reader& reader);
// 진행 중인가. 화면에서 버튼을 잠그는 데 쓴다.
bool companion_register_busy();
// 사용자에게 보일 마지막 상태. 빈 문자열이면 아직 없다.
