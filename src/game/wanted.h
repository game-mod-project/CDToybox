...
//   +0x48  목격자 벡터 크기
//   +0x58  범죄기록 벡터 크기
// --- 구역 이름: **파생이 아니라 실측 표다** (2026-09-20) ------------------
//
// 키에서 이름을 끌어내는 길을 못 찾았다. 2026-09-20 에 이론 넷을 반증했다.
//
//   1. `RegionInfo._displayRegionName`(+0x18) - 그 표에 1000131 이 없다
//      (1007행, `+0x00` 은 순번도 1000xxx 키도 아니다)
//   2. `FactionInfo` 키 - 135행, 키 범위가 다르다
//   3. 현지화 엔티티 = 수배 키 - 엔티티 1000131 은 **아이템**이다
//      (필드 0x70 이 "현상금 사냥꾼 가죽 투구")
//   4. `RegionInfoManager` 의 곁배열 넷 - 1000131 이 어디에도 없다
//
// 확실한 것은 하나다 - **현지화 엔티티 61 · 필드 0x92 = "에르난드 공국"**.
// `1000131 -> 61` 을 잇는 자리를 못 찾았다.
//
// 그래서 **화면에서 직접 확인된 짝만** 적는다. 추측으로 채우지 않는다 -
// 2026-09-19 에 "0x00..0x80 을 훑어 처음 비지 않은 문자열" 로 지었다가
// 데메니스 왕국이 **"구매하기({price})"** 로 나왔다.
//
// 화면에는 **키를 먼저** 내고 이름을 뒤에 붙인다. 틀린 줄이 있으면 키와
// 나란히 보여 바로 눈에 띈다. 모르는 키는 이름 없이 키만 낸다.
//
// 남은 셋(페일론 연합국 · 델레시아 공화국 · 붉은사막)의 키는 아직 못 봤다.
// 로그의 `벌금: … 구역 N` 줄에 키가 찍히므로, 그 지역에서 수배가 붙으면
// 저절로 모인다.
