...
// ⚠️ **두 가지를 반드시 알아야 한다.**
//
//  1. 슬롯의 "지금 고른 카테고리" 는 런타임 레코드 **`+0xD8` 한 칸**뿐이라 한
//     슬롯에 셋을 넣으면 셋이 그 칸을 다툰다(실측: 드래곤을 고르자 5 -> 2).
//  2. **끄고 나서 게임을 재시작해야 원래대로 돌아온다.** 정적 표는 즉시
//     원복되지만 게임 쪽 휠은 그 목록을 보고 만들어 둔 상태를 들고 있다 -
//     되돌린 뒤에도 특수 탑승물이 먹통이었고, 재시작 한 번으로 돌아왔다
//     (사용자 확인 2026-09-15). TROUBLESHOOTING 3.12.
