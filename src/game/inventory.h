...
//
// 그래서 지금은 **`기본 = (+0x14) - ((+0x18) + (+0x1A))`** 로 유도하고, 두 갈래 중
// 고른 한 칸만 바꾼다(반대 칸은 그대로 둔다).
//
// **`+0x16`(합계)은 읽지도 쓰지도 않는다**(2026-09-13 재설계). 실측으로 두 번
// 재현된 것: 리로드에서 게임은 `+0x14` 와 두 갈래는 되돌리면서 `+0x16` 만 우리 값을
// 그대로 둔다. 그 칸은 **세이브에도 실린다**(게임을 완전히 껐다 켜도 남았다). 즉
// 우리가 세이브에 남기는 유일한 값이 우리가 원치 않는 값이었고, 그 낡은 합계 때문에
// 우리 모델이 제 컨테이너를 영영 거부했다(`cap < sum`).
//
// **게임이 그 칸을 읽지 않는다는 것은 가정이지 증거가 아니다.** 관측한 것은 "로드에서
// 그 칸을 되돌리지 **않는다**" 뿐이고, 그것은 "읽지 않는다" 와 다른 명제다(세이브에서
// 복원하지 않는 것만으로도 같은 관측이 나온다). 가정이 틀리면 엔진이 `+0x14` 를 그
// 칸에서 재계산할 수 있고, 그때는 아래 항등식이 아무것도 못 막는다 - 그 재계산에
// 우리가 쓴 숫자가 하나도 안 들어가기 때문이다.
//
// 지금 세이브에 남아 있을 수 있는 값은 **옛 우리 코드가 쓴 것뿐**이고 그것은 전부
// `base + sum <= 700` 이라 732 를 못 넘는다. 위험은 "앞으로 그 칸에 무엇이 들어가든
// 우리는 모른다" 쪽이다.
//
// 두 갈래에서 유도하면 **낡은 합계가 있어도 맞는 값이 나온다**(가방 240/464/190/0
// 에서도 기본 50). 그래서 이미 손상된 세이브도 그냥 통과한다.
//
// **우리가 `+0x14` 에 쓰는 값**의 상한은 산술이 구조적으로 지킨다:
// `expand = want - base - other` 이므로 `capacity = base + other + expand = want` 다.
// 기본 슬롯 유도가 틀려도 그 값은 언제나 정확히 want 이고, want 는 700/732/물리
// 배열로 잘린다. 다만 그 항등식이 묶는 것은 **거기까지**다 - 엔진이 제 방식으로
// 재계산하는 값도, 고른 갈래에 우리가 덮어쓰는 값이 옳은가도 묶지 않는다.
// 뒤엣것은 아래 base_max 가 막는다.
//
...
struct BagKindRule {
    std::uint16_t kind;
    int cap;             // 이 종류의 용량 상한
    // **이 종류의 기본 슬롯**(실측). 유도값이 이보다 크면 우리가 잘못 읽은 것이다 -
    // 갈래가 아직 안 채워진 컨테이너를 보면 `base = cap - 0` 이 되어 참값보다 훨씬
    // 커진다. 옛 모델은 그 순간을 `a+b != sum` 으로 막았는데, 합계를 안 읽게 되면서
    // 그 관문이 사라졌다. 이것이 그 자리를 대신한다(검토 치명 1).
    //
    // 정당한 확장은 갈래에 들어가므로 기본 슬롯은 변하지 않는다 - 확장 전이든
    // 후든, 우리가 늘린 뒤든 언제나 같은 값이다.
    int base_max;
...
// 이 종류의 실측 기본 슬롯. 표에 없으면 0(= 검사하지 않는다).
...
struct BagPlan {
    ...
    int base = 0;              // 유도한 기본 슬롯 = cap - ((+0x18) + (+0x1A))
    int branch = kBagBranchA;  // 어느 칸을 쓸 것인가
    int expand = 0;            // 고른 칸에 쓸 값
    int other = 0;             // 반대 칸의 현재 값(그대로 둔다)
    int capacity = 0;          // +0x14 에 쓸 값 = base + sum
};

// cap/a/b 는 각각 컨테이너의 +0x14/+0x18/+0x1A, slots 는 +0x08 이다.
// **+0x16 은 인자에 없다** - 읽지 않기 때문이다(위 큰 주석 참고).
...
// ------------------------------------------------------------- 손상 복구
//
// **우리가 어긋뜨린 합계(+0x16)를 갈래 합으로 되돌린다 - 이제는 미용이다.**
// 새 산술은 그 칸을 읽지도 쓰지도 않으므로 낡은 합계가 기능을 막지 않는다. 다만
// 옛 코드가 세이브에 남긴 값이라 치울 수단은 있어야 한다(그 칸에 우리 값이 남는
// 것은 실측으로 확인됐다 - 게임 재시작을 넘어 464·650 이 살아 왔다).
//
...
struct BagBackup {
    ...
    std::uint16_t cap = 0, a = 0, b = 0;   // **최초** 원본, 덮지 않는다
    ...
    //      (재검토 치명 1).
    // want_exp 는 **두 갈래의 합**이다(+0x16 이 아니다 - 우리는 그 칸을 안 쓴다).
    std::uint16_t want_cap = 0, want_exp = 0;
};
...
struct BagSeen {
    ...
    std::uint16_t cap = 0, a = 0, b = 0;   // 손대기 **전** 지금 값
    ...
    std::uint16_t want_cap = 0, want_exp = 0;      // known 일 때의 그 값
};
...
// cap/exp/used 는 지금 컨테이너의 +0x14 / (+0x18)+(+0x1A) / +0x12 다.
...
// 명분이 없어진 뒤에도 자동으로 계속할 이유가 없다(검토 중대 3). 치우는 것은
// 화면 버튼(bag_repair)으로만 한다.
//
