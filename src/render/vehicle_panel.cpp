...
void draw_vehicle_panel(bool* open) {
    ...
    if (collapsing_header("veh.call", "소환 (호출 장소)")) {
        ...
        // 행동 제한 목록(`actionlimit.h`, 읽기만). 어느 곳이 무슨 제한을 거는지
        // 보인다. 여기서 풀지는 않는다 - 허용 목록을 넘기는 토글을 넣었다가 보스룸에서
        // 효과가 없어 걷어냈다(2026-09-21, specs/2026-09-21-boss-room-action-limit.md).
        ImGui::Separator();
        const game::ActionLimitState bc = game::action_limit_state(reader);
        if (!bc.player) {
        ...
        if (mv.empty()) {
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip(
                    ...
                    " 회색으로 잠기면 눌러 주십시오.\n"
                    "힙을 통째로 훑어 1분쯤 걸리고(실측 44~68초, 판마다"
                    " 다릅니다), 분석 스레드에서 도므로 게임은 안 멈춥니다.");
            }
            ImGui::SameLine();
