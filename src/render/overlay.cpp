...
void on_frame(IDXGISwapChain3* sc, ID3D12CommandQueue* queue) {
    ...
    // freeze 가 멈춰 무적이 안 먹었다. 발견/게이지 캐시는 분석 루프가 담당.
    {
        ...
        //
        // **설정으로 켤 때만 건다.** 처음에는 상시로 걸면서 주석에 "형제들과
        // 같은 순수 통과 훅이니 괜찮다" 고 적었다 - 근거 없이 예외를 만든
        // 것이다. 바로 아래 `dragondiag` 를 잠가 둔 이유가 그대로 여기에도
        // 걸린다: 조사용 훅은 기능이 아니고, 게임 함수 한복판에 상시로 앉을
        // 이유가 없다. 거부가 **없는** 것도 결과라 횟수도 같이 알린다.
        if (g_cfg.call_diag) {
