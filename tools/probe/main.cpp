...
//
// 배포 전에 여기서 확인한다 - 모드와 **같은 코드**(game/mountvital.cpp)를 쓴다.
void cmd_mountauth(mem::Rtti& rt, const mem::Reader& reader, int argc,
                   char** argv) {
    ...
    p.hp_cur = std::strtoll(argv[3], nullptr, 0);
    p.hp_max = std::strtoll(argv[4], nullptr, 0);
    p.sta_cur = std::strtoll(argv[5], nullptr, 0);
    p.sta_max = std::strtoll(argv[6], nullptr, 0);
    std::printf("\n핸들 %08X 에 씁니다\n", h);
    if (!game::mount_vital_write(reader, h, p)) {
        std::printf("쓰기 실패\n");
        return;
    }
    for (int i = 0; i < 10; ++i) {
...
int main(int argc, char** argv) {
    ...
    if (cmd == "mountauth") { cmd_mountauth(rt, reader, argc, argv); return 0; }
