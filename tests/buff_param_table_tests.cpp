...
TEST(buff_param_table_has_the_three_measured_rules) {
    ...
    CHECK_EQ(buff_param_divisor(0x1458FBD20ull, 1, 0), 1000.0);      // 생명 최대치 45
    CHECK_EQ(buff_param_divisor(0x1458FA908ull, 1, 0), 10000.0);     // 치명타 2/5/7%
    CHECK_EQ(buff_param_divisor(0x1458FB5A0ull, 2, 1), 10000000.0);  // 용기 75% 회복
    CHECK_EQ(buff_param_offset(0x1458FBD20ull, 1, 0), 0x98);
    CHECK_EQ(buff_param_offset(0x1458FA908ull, 1, 0), 0x98);
    CHECK_EQ(buff_param_offset(0x1458FB5A0ull, 2, 1), 0x98);
}
...
TEST(buff_param_table_splits_one_class_by_the_buffdata_flag) {
    ...
    CHECK_EQ(buff_param_divisor(0x1458FB5A0ull, 2, 1), 10000000.0);
    CHECK_EQ(buff_param_divisor(0x1458FB5A0ull, 2, 0), 1000.0);
    CHECK(buff_param_divisor(0x1458FB5A0ull, 2, 0)
          != buff_param_divisor(0x1458FB5A0ull, 2, 1));
    // 같은 갈래가 걸린 다른 클래스들(비율 가족)도 같은 모양이다.
    CHECK_EQ(buff_param_divisor(0x1458FBD88ull, 1, 0), 1000.0);       // VaryStat
    CHECK_EQ(buff_param_divisor(0x1458FBD88ull, 1, 1), 10000000.0);
    CHECK_EQ(buff_param_divisor(0x1458FC230ull, 1, 0), 1000.0);       // VaryDataDefinedStat
    CHECK_EQ(buff_param_divisor(0x1458FC230ull, 1, 1), 10000000.0);
    CHECK_EQ(buff_param_divisor(0x1458FB610ull, 1, 0), 1.0);          // VaryStatRate
    CHECK_EQ(buff_param_divisor(0x1458FB610ull, 1, 1), 10000.0);
    // 안 갈리는 클래스는 두 값이 같아야 한다 — 갈래를 잘못 붙이지 않았는지.
    CHECK_EQ(buff_param_divisor(0x1458FBD20ull, 1, 1), 1000.0);
    CHECK_EQ(buff_param_divisor(0x1458FA908ull, 1, 1), 10000.0);
}

TEST(buff_param_table_splits_one_class_by_the_parameter_type) {
    // Damage 는 종류마다 다른 칸을 쓴다. 종류를 안 보면 남의 칸을 읽는다.
    CHECK_EQ(buff_param_offset(0x1458FC370ull, 1, 0), 0x98);
    CHECK_EQ(buff_param_divisor(0x1458FC370ull, 1, 0), 1000.0);
    CHECK_EQ(buff_param_offset(0x1458FC370ull, 3, 0), 0xF8);
    CHECK_EQ(buff_param_divisor(0x1458FC370ull, 3, 0), 10000.0);
    // SetStatMinRate 도 마찬가지다(+0x98 / +0xA0).
    CHECK_EQ(buff_param_offset(0x1458FD728ull, 1, 0), 0x98);
    CHECK_EQ(buff_param_offset(0x1458FD728ull, 2, 0), 0xA0);
}

TEST(buff_param_table_covers_the_absolute_value_twins) {
    // {|ParamN|} 은 종류 N+4 로 온다. 같은 칸·같은 배율이어야 한다.
    for (std::uint8_t t = 0; t < 4; ++t) {
        const BuffParamRule* plain = buff_param_rule(0x1458FBD20ull, t, 0);
        const BuffParamRule* twin =
            buff_param_rule(0x1458FBD20ull, static_cast<std::uint8_t>(t + 4), 0);
...
TEST(buff_param_table_reports_unknown_combinations) {
    ...
    CHECK_EQ(buff_param_divisor(0x1458FBD20ull, 2, 0), 0.0);
    CHECK(buff_param_class_known(0x1458FBD20ull));
...
TEST(buff_param_table_treats_any_flag_other_than_one_as_zero) {
    ...
    CHECK_EQ(buff_param_divisor(0x1458FB5A0ull, 2, 7), 1000.0);
}

TEST(buff_param_table_leaves_classes_without_a_parameter_out) {
    // 슬롯 11 이 빈 문자열만 돌려주는 클래스들. 추측해 넣으면 안 된다.
    CHECK(!buff_param_class_known(0x1458FB818ull));   // VoidPassiveBuffData
    CHECK(!buff_param_class_known(0x1458FB880ull));   // VoidActiveBuffData
    CHECK(!buff_param_class_known(0x1458FC2A0ull));   // ImmuneBuffData
    // 파라미터가 숫자가 아니라 표 조회(이름)뿐인 것도 마찬가지다.
    CHECK(!buff_param_class_known(0x1458FADE8ull));   // IgnoreUseResourceStat
}
...
TEST(buff_param_table_keeps_the_level_classes_at_their_raw_value) {
    ...
    CHECK_EQ(buff_param_offset(0x1458FD040ull, 1, 0), 0x94);   // ChangeBuffLevel
    CHECK_EQ(buff_param_divisor(0x1458FD040ull, 1, 0), 1.0);
    CHECK_EQ(buff_param_offset(0x1458FC1C0ull, 1, 0), 0xA0);   // VaryStaticStatLevel
...
TEST(buff_param_table_is_usable_at_compile_time) {
    ...
    static_assert(buff_param_divisor(0x1458FBD20ull, 1, 0) == 1000.0);
    static_assert(buff_param_offset(0x1458FBD20ull, 1, 0) == 0x98);
    static_assert(buff_param_divisor(0x1458FB5A0ull, 2, 0) == 1000.0);
    static_assert(buff_param_divisor(0x1458FB5A0ull, 2, 1) == 10000000.0);
